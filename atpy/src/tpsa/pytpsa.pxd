...
cdef extern from "./include/da.h":
    cdef cppclass DAVector:
        ...
        const int n_element() 
        const void element(unsigned int i, unsigned int *c, double& elem) 
        const void element(unsigned int i, vector[unsigned int]& c, double& elem)
        double element(int i) 
        double element(vector[int] idx) 
        void set_element(int *c, double elem) 
        void set_element(vector[int] idx, double elem) 
        void reset() 
...
#     * The i-th base can be accessed as base[i], where base is an object of Base.
    ...
    void da_substitute(const DAVector &iv, vector[unsigned int] &base_id, vector[DAVector] &v, DAVector &ov) 
    void da_substitute(vector[DAVector] &ivecs, vector[unsigned int] &base_id, vector[DAVector] &v,
                  vector[DAVector] &ovecs) 
    void da_composition(vector[DAVector] &ivecs, vector[DAVector] &v, vector[DAVector] &ovecs) 
    void da_composition(vector[DAVector] &ivecs, vector[double] &v, vector[double] &ovecs) 
    ...
    void inv_map(vector[DAVector] &ivecs, int dim, vector[DAVector] &ovecs) 
