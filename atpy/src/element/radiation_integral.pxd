...
cdef inline void calc_sync_int(double rhoinv,double blen,double k1,double e1,double e2,double betxi,double alfxi,double dxi,double dpxi,double* glb)nogil:
    '''
        !----------------------------------------------------------------------*
        !     Purpose:                                                         *
    ...
    if (rhoinv != 0.0):
        ...
        glb[RI3 ] += fabs(rhoinv)**3 * blen
        glb[RI3A] += rhoinv**3*blen
        glb[RI4 ] += dispaverage*rhoinv*(rhoinv**2 + 2*k1) * blen \
               - rhoinv*rhoinv*(dx*tan(e1) + dx2*tan(e2))
        glb[RI5 ] += curlyhaverage * fabs(rhoinv)**3 * blen
    ...
    if(k1 != 0.0):
        ...
        glb[RI6] += (k1n**2)*betxaverage*lq
        glb[RI7] += (k1n**2)*dispaverage**2*lq
